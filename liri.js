  ...
  request(URL, function(err, res, body) {
    ...
    if (data && !data.Error) {
      var output = [
        ...
        "Year: " + data.Year,
        "Ratings:",
        "- IMDB: " + (data.Ratings.length && data.Ratings[0])
          ? data.Ratings[0].Value
          : "N/A",
        "- Rotten Tomatoes: " + (data.Ratings.length && data.Ratings[1])
...
var Concert = function(artist) {
  const URL = `https://rest.bandsintown.com/artists/${encodeSpecialChars(
    artist
...
var Song = function(song) {
  spotify.search({ type: "track", query: song ? song : "The Sign" }, function(
    err,
    body
  ) {
    if (err) throw err;
    ...
    if (data) {
      ...
      fs.appendFile(
        "log.txt",
        `Could not find a match for "${song}" on Spotify\n`,
        err => {
          if (err) throw err;
        }
      );
...
fs.appendFile("log.txt", process.argv.join(" "), err => {
  if (err) throw err;
});
